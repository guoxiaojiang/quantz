...
def growing_value_filter_func(stocks: pd.DataFrame, ts_token, when, or_yoy, rd_exp_min, rd_exp_max,
                              o_exp, gpr):
    logv('growing value filter or_yoy=%s rd_exp_min=%s rd_exp_max=%s o_exp=%s grp=%s'
         % (or_yoy, rd_exp_min, rd_exp_max, o_exp, gpr))
...
def run(ts_tokens, rd_exp_min, rd_exp_max, or_yoy, o_exp, gross_profit_rate):
    log_init(0)
    logi('Growing Value picker running\nor_yoy=%d,rd_exp_min=%s rd_exp_max=%s o_exp=%s,gross_profit=%s\n'
